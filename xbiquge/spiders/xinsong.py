...
class SancunSpider(scrapy.Spider):
    name = 'xinsong'
    allowed_domains = ['www.sanguoxs.net']
    #start_urls = ['https://www.xbiquge.la/10/10489/']
    url_ori= "http://www.sanguoxs.net"
    url_firstchapter = "http://www.sanguoxs.net/xinsong/388984.html"
    name_txt = "./novels/新宋"
...
#        pdb.set_trace()
    ...
    def parse(self, response):    #网页提取数据，并与mongodb数据集比较，没有相同的数据才从网页抓取。
        ...
        dl = response.css('#play_0 ul li')     #提取章节链接相关信息
        for dd in dl:
            count_iterator = 0
            self.url_c = dd.css('a::attr(href)').extract()[0]   #组合形成小说的各章节链接
    ...
    def parse_c(self, response):
        ...
        self.item['preview_page'] = response.css('body div div div.m-page a::attr(href)').extract()[0]
        self.item['next_page'] = response.css('body div div div.m-page a::attr(href)').extract()[2]
        title = response.css('body > div > div > div.m-title.col-md-12 > h1::text').extract()[0]
