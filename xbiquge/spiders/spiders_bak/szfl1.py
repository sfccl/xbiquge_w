...
class SancunSpider(scrapy.Spider):
    name = 'szfl1'
    allowed_domains = ['www.yqxs.cc']
    #start_urls = ['https://www.xbiquge.la/10/10489/']
    url_ori= "https://www.yqxs.cc"
    url_firstchapter = "https://www.yqxs.cc/html/86340/86340932/110143355.html"
    name_txt = "./novels/世子风流1"
    url_chapters = url_firstchapter[0:40]  #截取字符串包括尾部的正斜杠
...
#        pdb.set_trace()
    ...
    def parse(self, response):    #网页提取数据，并与mongodb数据集比较，没有相同的数据才从网页抓取。
        ...
        dl = response.css('div.listmain > dl > dd')     #提取章节链接相关信息
        for dd in dl:
            count_iterator = 0
    ...
    def parse_c(self, response):
        ...
        self.item['preview_page'] = self.url_ori + response.css('#wrapper > div.book.reader > div.content > div.page_chapter > ul > li:nth-child(1) > a::attr(href)').extract()[0]
        self.item['next_page'] = self.url_ori + response.css('#wrapper > div.book.reader > div.content > div.page_chapter > ul > li:nth-child(3) > a::attr(href)').extract()[0]
        title = response.css('#wrapper > div.book.reader > div.content > h1::text').extract()[0]
        ...
        if self.item['url'][40:49] == self.item['next_page'][40:49]: #同一章有分页的处理
