...
class SancunSpider(scrapy.Spider):
    name = 'yfqtx'
    allowed_domains = ['www.luoqiuzw.com']
    #start_urls = ['http://www.xbiquge.la/10/10489/']
    url_ori= "https://www.luoqiuzw.com"
    url_firstchapter = "https://www.luoqiuzw.com/book/94819/58420795.html"
    name_txt = "./novels/医妃倾天下"
    url_chapters = url_firstchapter[0:36]
...
#        pdb.set_trace()
    ...
    def parse_c(self, response):
        ...
        title = response.css('.con_top::text').extract()[3]
        contents = response.css('div #content p.content_detail::text').extract()
        text=''
        for content in contents:
            text = text + content
        #print(text)
#        self.item['content'] = title + "\n" + contents.replace('<p class="content_detail">','').replace('</p>','').replace('<br>','')
        self.item['content'] = title + "\n" + text.replace('\15', '\n').replace('\n\n                \n\n            \n\n','\n')     #各章节标题和内容组合成content数据，\15是^M的八进制表示，需要替换为换行符。
        yield self.item     #以生成器模式（yield）输出Item对象的内容给pipelines模块。

        if self.item['url'][36:45] == self.item['next_page'][36:45]: #同一章有分页的处理
