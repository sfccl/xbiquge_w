...
class SancunSpider(scrapy.Spider):
    name = 'szfl'
    allowed_domains = ['www.ff96.net']
    #start_urls = ['https://www.xbiquge.la/10/10489/']
    url_ori= "https://www.ff96.net"
    url_firstchapter = "https://www.ff96.net/125/125125/51363982.html"
    name_txt = "./novels/世子风流"
...
#        pdb.set_trace()
    ...
    def parse(self, response):    #网页提取数据，并与mongodb数据集比较，没有相同的数据才从网页抓取。
        ...
        dl = response.css('div.warpper > div.mu_contain > ul.mulu_list > li')     #提取章节链接相关信息
        for dd in dl:
            count_iterator = 0
    ...
    def parse_c(self, response):
        ...
        self.item['preview_page'] = self.url_ori + response.css('#content > tbody > tr > td > div.yd_linebot > div > span:nth-child(1) > a::attr(href)').extract()[0]
        self.item['next_page'] = self.url_ori + response.css('#content > tbody > tr > td > div.yd_linebot > div > span:nth-child(3) > a::attr(href)').extract()[0]
        title = response.css('#content > tbody > tr > td > h1::text').extract()[0]
        contents = response.css('#htmlContent::text').extract()
        text=''
        ...
        if self.item['url'][32:40] == self.item['next_page'][32:40]: #同一章有分页的处理
