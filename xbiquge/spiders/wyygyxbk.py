...
class SancunSpider(scrapy.Spider):
    ...
    url_ori= "http://www.xqdhk.com"
    url_firstchapter = "http://www.xqdhk.com/c483070/1.html"
    name_txt = "./novels/我有一个医学宝库1"
    ...
    url_chapters = "http://www.xqdhk.com/c483070/1/"
...
#        pdb.set_trace()
    ...
    def parse(self, response):    #网页提取数据，并与mongodb数据集比较，没有相同的数据才从网页抓取。
        ...
        dl = response.css('body > div.container.autoheight > div.list-chapter > div.booklist > ul > li')     #提取章节链接相关信息
        for dd in dl:
            count_iterator = 0
    ...
    def parse_c(self, response):
        ...
        preview_page_t = response.css('#wrapper > div.container.autoheight > div.book.read > div:nth-child(3) > a::attr(href)').extract_first()
        preview_page_t = response.urljoin(preview_page_t)
        self.item['preview_page_t'] = preview_page_t
        next_page_t = response.css('#wrapper > div.container.autoheight > div.book.read > div:nth-child(3) > a:nth-child(4)::attr(href)').extract_first()
        next_page_t = response.urljoin(next_page_t) 
        self.item['next_page'] = next_page_t
        #self.item['next_page'] = self.url_ori + response.css('div .bottem1 a::attr(href)').extract()[3]
        #title = response.css('.con_top::text').extract()[4]
        title = response.css('#wrapper > div.container.autoheight > div.book.read > h1::text').extract_first()
