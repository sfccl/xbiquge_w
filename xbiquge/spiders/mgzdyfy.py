...
class SancunSpider(scrapy.Spider):
    name = 'mgzdyfy'
    allowed_domains = ['www.wmdown.com']
    #start_urls = ['https://www.xbiquge.la/10/10489/']
    url_ori= "https://www.wmdown.com"
    url_firstchapter = "https://www.wmdown.com/novel/WJxbokKAbgw/1.html"
    name_txt = "./novels/民国之谍影风云"
    ...
    url_chapters = url_firstchapter[0:index_FS] + '.html' #在wmdown网站，该目录页需要加.html后缀
...
#        pdb.set_trace()
    ...
    def parse(self, response):    #网页提取数据，并与mongodb数据集比较，没有相同的数据才从网页抓取。
        ...
        dl = response.css('body > div.listmain > dl > dd')     #提取章节链接相关信息
        for dd in dl:
            count_iterator = 0
    ...
    def parse_c(self, response):
        ...
        #self.item['preview_page'] = self.url_ori + response.css('body > div.book.reader > div.content > div.page_chapter > ul > li > a::attr(href)').extract()[0]
        self.item['preview_page'] = urljoin(self.url_ori, response.css('div.page_chapter > ul > li > a::attr(href)').extract()[0])
        #self.item['next_page'] = self.url_ori + response.css('body > div.book.reader > div.content > div.page_chapter > ul > li > a::attr(href)').extract()[2]
        self.item['next_page'] = urljoin(self.url_ori, response.css('div.page_chapter > ul > li > a::attr(href)').extract()[2])
        title = response.css('body > div.book.reader > div.content > h1::text').extract()[0]
